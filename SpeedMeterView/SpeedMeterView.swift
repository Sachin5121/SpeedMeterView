...
@IBDesignable public class SpeedMeterView: UIView {
    ...
    //    drawLayer.frame = meterView.bounds
        let center = CGPoint (x: meterView.frame.size.width / 2, y: meterView.frame.size.height / 2)
        let circleRadius = meterView.frame.size.width / 2
        let startAngle: CGFloat = .pi
        let endAngle: CGFloat = .pi * 2
        
//        let circlePath = UIBezierPath(arcCenter: center, radius: circleRadius, startAngle: startAngle, endAngle: endAngle, clockwise: true)
        ...
        drawLayer.path = circlePath.cgPath
        
        drawLayer.strokeColor = UIColor.black.cgColor
        drawLayer.fillColor = UIColor.clear.cgColor
        drawLayer.lineWidth = 8
